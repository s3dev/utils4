...
class Database(object):
    ...
    def __del__(self):
        """Destroy the object(s) on garbage collection."""
    ...
    def run_query(self, query) -> list:
        ...
        This function runs the query that is passed as a string and
        returns the results as a list of rows.

        Args:
            query (str): SQL query to be run.

        Returns:
            If results were found, a populated list of rows is returned,
            otherwise a blank list.
        ...
        try:
            ...
            # USER NOTIFICATION
            self._ui.print_alert('\nAn error occurred whilst running the following query:\n\n'
    ...
    def _table_exists(self, sql) -> bool:
        """
        Test if a table exists, using the script provided.
...
class SQLite(Database):
    ...
    def _match(self, pattern, string):
        """Wrap ``re.match()`` as a user-defined function."""
        try:
            return re.match(pattern=pattern, string=string) is not None
        except Exception as err:
            # USER NOTIFICATION
            self._ui.print_alert('\nAn error occurred whilst creating the _match user-defined '
                                 'function.')
            self._ui.print_error(err)
            return False
